 ...
 *
 * The third binding is the outline's alone (addendum 19 §2): a Chapter row is
 * a `chapter` marker once promoted, and a board node has no such thing — so
 * it is optional here, and a node simply never matches a marker.
 */
interface Bound {
  boundUnitId: StructuralUnitId | null;
  boundBeatId: BeatId | null;
  boundMarkerId?: StoryMarkerId | null;
}

/** What a plan may be bound to, named one way everywhere the pair is discussed. */
export type BindingTarget = { unitId: StructuralUnitId } | { beatId: BeatId } | { markerId: StoryMarkerId };

const hitsTarget = (row: Bound, target: BindingTarget): boolean =>
  'unitId' in target
    ? row.boundUnitId === target.unitId
    : 'beatId' in target
      ? row.boundBeatId === target.beatId
      : (row.boundMarkerId ?? null) === target.markerId;
...
export const claimedInScript = (
...
): { units: Set<string>; beats: Set<string>; markers: Set<string> } => {
  ...
  const take = (id: string, row: Bound) => {
    ...
    if (row.boundMarkerId) markers.add(row.boundMarkerId as string);
  };
 ...
 */
export const retitleScript = (file: ProjectFile, target: BindingTarget, title: string): ProjectFile => {
  const at = nowIso();
  if ('unitId' in target) {
  ...
  if ('markerId' in target) {
    if (!file.markers.some((marker) => marker.id === target.markerId)) return file;
    return {
      ...file,
      markers: file.markers.map((marker) =>
        marker.id === target.markerId ? { ...marker, title, updatedAt: at } : marker,
      ),
      project: { ...file.project, updatedAt: at },
    };
  }
...
/**
 * Rename whatever is bound to a scene, a beat or a chapter, from the script.
 *
 * The other half of "rename it in either place". Called by `updateUnit`,
 * `updateBeat` and `updateMarker` when a title changes, so a scene renamed in
 * the workspace is renamed on **every board and in every outline** that says
 * it is real — because the plan and the script are the same object, and there
 * is no version of this where two of them disagree.
 */
export const retitlePlans = (file: ProjectFile, target: BindingTarget, title: string): ProjectFile => {
  const at = nowIso();
  const hits = (row: Bound): boolean => hitsTarget(row, target);
 ...
 *
 * Called by `removeUnit`, `removeBeat` and `removeMarker`. A card pointing at
 * something that no longer exists is worse than a plan — it claims to be real
 * and cannot say what it is — so it goes back to being a plan, which is what
 * it was before the writer bound it. Nothing else about it changes, on either
 * the board or the outline.
 */
export const unbindRemovedFromPlans = (
  file: ProjectFile,
  removed: { units: Set<string>; beats: Set<string>; markers?: Set<string> },
): ProjectFile => {
  const at = nowIso();
  let changed = false;
  ...
  const letGo = <T extends Bound & { updatedAt: string }>(row: T): T => {
    ...
    const markerGone = Boolean(row.boundMarkerId) && (removed.markers?.has(row.boundMarkerId as string) ?? false);
    if (!unitGone && !beatGone && !markerGone) return row;
    changed = true;
    return {
      ...row,
      boundUnitId: unitGone ? null : row.boundUnitId,
      boundBeatId: beatGone ? null : row.boundBeatId,
      ...(markerGone ? { boundMarkerId: null } : {}),
      updatedAt: at,
    };
  };
